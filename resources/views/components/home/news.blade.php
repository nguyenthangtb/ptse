            ...
            @foreach($news ?? [] as $article)
                <a href="{{ route('news.show', $article) }}" class="group relative bg-secondary rounded-lg shadow-md overflow-hidden reveal block transform transition-all duration-500 hover:-translate-y-1 hover:shadow-xl">
                    <div class="relative h-[300px] sm:h-[350px] overflow-hidden">
                        @if($article->image)
                            <img src="{{ Storage::url($article->image) }}" alt="{{ $article->title }}" class="w-full h-full object-contain transition-all duration-700 ease-in-out group-hover:scale-110 group-hover:rotate-1">
                        @else
                            <img src="https://placehold.co/800x400" alt="{{ $article->title }}" class="w-full h-full object-contain transition-all duration-700 ease-in-out group-hover:scale-110 group-hover:rotate-1">
                        @endif
